...
def test_false_strings_remain_false_and_unknown_labels_raise():
    raw = _vendor()
    raw["fault_confirmed"] = ["false"] * len(raw)
    adapter, _ = Adapter.suggest(raw, "door")
    out, _ = adapter.apply(raw)
    assert not out.fault_confirmed.any()
    raw["fault_confirmed"] = "maybe"
    with pytest.raises(ValueError, match="boolean"):
        adapter.apply(raw)


def test_declared_units_are_converted_before_integral_derivation():
    raw = _vendor().drop(columns=["charge"])
    adapter, _ = Adapter.suggest(raw, "door")
    out_before, _ = adapter.apply(raw)
    adapter.unit_scales = {"mean_current_a": .001}
    out, _ = adapter.apply(raw)
    np.testing.assert_allclose(out.current_integral_as, out_before.current_integral_as * .001)


def test_context_placeholders_are_machine_readable():
    raw = _vendor().drop(columns=["saloon_temp", "load_weigh"])
    out, _ = Adapter.suggest(raw, "door")[0].apply(raw)
    assert not out.context_supported.any()
